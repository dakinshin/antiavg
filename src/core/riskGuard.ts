...
export class RiskGuard {
  ...
  private track<T>(p: Promise<T>): Promise<T> {
    ...
    void p.catch(() => undefined).finally(() => this.pendingWork.delete(p));
    return p;
  }
  ...
  private async ensureDefaultStop(symbol: string, positionSide: PositionSide): Promise<void> {
    if (this.stopped || !this.cfg.defaultStopEnabled) return;
    const pos = this.opts.positions.peek(symbol, positionSide);
    if (!pos || isZero(pos.qty)) return;

    // За время ожидания человек мог поставить стоп сам — это штатный исход.
    if (this.stopsFor(symbol, positionSide).length > 0) {
      this.log.info('стоп выставлен вручную, дефолтный не нужен', { symbol, positionSide });
      return;
    }
