 ...
 *
 * Признаки FOMO считает `FomoDetector` — серию коротких стоп-аутов и пачку
 * убыточных входов; здесь решается, что с ними делать. Достаточно любого из
 * двух. Три режима, ровно как просил человек:
 *
 *   off    — выключено;
...
import { FomoDetector, type ClosedTrade, type FomoReason } from './fomo.js';
...
export interface FomoTriggerInfo {
  /** Какой признак сработал: серия стоп-аутов или пачка убыточных входов. */
  reason: FomoReason;
  /** Сделки, образовавшие признак. */
  trades: ClosedTrade[];
...
export class FomoGuard {
  ...
  constructor(private readonly opts: FomoGuardOptions) {
    ...
    this.detector = new FomoDetector({
      ...
      minLossPct: this.cfg.fomoMinLossPct,
      burstWindowMs: this.cfg.fomoBurstWindowMs,
      burstCount: this.cfg.fomoBurstCount,
      // Пачку помним, пока она может на что-то повлиять: окно набора плюс
      // время блокировки. Дальше реагировать поздно — если мы на неё
      // блокировали, блокировка уже прошла; если нет, помогать некому.
      burstRetentionMs: this.cfg.fomoBurstWindowMs + this.cfg.fomoBlockMs,
    });
  }
  ...
  onPositionClosed(trade: ClosedTrade): void {
    ...
    this.log.debug('сделка закрыта, признаки FOMO пересчитаны', {
      ...
      серия: `${outcome.streak} из ${this.cfg.fomoStopLossCount}`,
      пачка: `${outcome.burst} из ${this.cfg.fomoBurstCount}`,
    });
    ...
    const first = outcome.trades[0]!;
    const last = outcome.trades[outcome.trades.length - 1]!;
    const text =
      outcome.reason === 'streak'
        ? `${outcome.trades.length} сделки подряд закрыты стопом в убыток за ` +
          `${Math.round((last.closedAtMs - first.closedAtMs) / 1000)} с — ${tail}`
        : `${outcome.trades.length} убыточные сделки открыты за ` +
          `${Math.round((Math.max(...outcome.trades.map((t) => t.openedAtMs!)) - Math.min(...outcome.trades.map((t) => t.openedAtMs!))) / 1000)} с — ${tail}`;

    this.log.error(
      outcome.reason === 'streak'
        ? 'ЗАЩИТА ОТ FOMO: серия коротких стоп-аутов'
        : 'ЗАЩИТА ОТ FOMO: пачка убыточных входов',
      {
        признак: outcome.reason,
        сделки: outcome.trades.map((t) => ({
          symbol: t.symbol,
          открытаВ: t.openedAtMs ? new Date(t.openedAtMs).toISOString() : 'неизвестно',
          закрытаВ: new Date(t.closedAtMs).toISOString(),
          прожилаМс: t.durationMs,
          результат: round8(t.pnl),
        })),
        окноМс: outcome.reason === 'streak' ? this.cfg.fomoWindowMs : this.cfg.fomoBurstWindowMs,
  ...
  stats(): {
    ...
    return {
      ...
      пачкаСейчас: this.detector.currentBurst(),
  ...
  state(): {
    ...
    return {
      ...
      burst: this.detector.currentBurst(),
    };
  }
