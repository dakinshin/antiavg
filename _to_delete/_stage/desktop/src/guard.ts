...
export class Guard {
  ...
  /**
   * Проверка замка перед выключением защиты.
   *
   * Если проверить не удалось (нет связи с биржей), возвращаем «не заперто»:
   * замок — это самоограничение, а не средство защиты. Превращать сетевой сбой
   * в невозможность выключить сервис нечестно — процесс всё равно можно снять
   * из диспетчера задач.
   */
  async checkDrawdownLock(settings: Settings): Promise<{
    locked: boolean;
    status?: DrawdownStatus;
    error?: string;
  }> {
    if (!settings.lockStopWhileInDrawdown || !this.app) return { locked: false };
    try {
