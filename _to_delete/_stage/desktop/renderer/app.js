...
const FIELDS = {
  ...
  sLockDrawdown: 'lockStopWhileInDrawdown', sSymbols: 'symbols', sMaxActions: 'maxActionsPerHour',
...
async function saveSettings() {
  ...
  // Часть полей мог откатить замок просадки — показываем то, что сохранилось.
  if (res.values) {
    for (const [id, key] of Object.entries(FIELDS)) {
      const node = $(id);
  ...
  if (res.needsRestart) {
    events.unshift({
      ...
      text: 'настройки сохранены — перезапустите защиту, чтобы они применились',
    });
    renderLog();
  }
}
