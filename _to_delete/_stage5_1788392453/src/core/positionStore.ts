...
export interface ApplyFillResult {
  ...
  addedQty: number;
  /**
   * Результат сделки в валюте котировки на момент её закрытия (или переворота):
   * сумма по всем закрывающим исполнениям. Осмыслен только когда `closed` или
   * `flipped`; в остальных случаях — накопленное на сейчас.
   */
  realizedPnl: number;
}
...
export class PositionStore {
  ...
  applyFill(
    ...
    let increased = false;
    let addedQty = 0;
    let tradePnl = p.realizedPnl;

    /**
     * Прибыль от закрытия `qty` единиц по цене `price`. Знак берётся от текущей
     * позиции: для шорта прибыльно движение вниз.
     */
    const closingPnl = (qty: number): number =>
      p.qty > 0 ? (price - p.entryPrice) * qty : (p.entryPrice - price) * qty;
    ...
    if (wasFlat) {
      ...
      // Полное закрытие.
      p.realizedPnl += closingPnl(Math.abs(p.qty));
      tradePnl = p.realizedPnl;
      p.qty = 0;
      p.entryPrice = 0;
      p.openedAtMs = null;
      p.openTimeKnown = true;
      ...
      // Частичное закрытие — средняя не меняется, но результат копится.
      p.realizedPnl += closingPnl(Math.abs(signedDelta));
      tradePnl = p.realizedPnl;
      p.qty = next;
    } else {
      // Переворот: старая сделка закрыта целиком, остаток открывается по цене
      // исполнения — и это уже новая сделка со своим счётом.
      p.realizedPnl += closingPnl(Math.abs(p.qty));
      tradePnl = p.realizedPnl;
      p.qty = next;
      p.entryPrice = price;
    ...
    return {
      before,
      after: { qty: p.qty, entryPrice: p.entryPrice },
      opened,
      closed,
      flipped,
      increased,
  ...
  applySnapshot(
    ...
    if (isFlat) {
      ...
      p.openTimeKnown = true;
      p.openedByOrderId = null;
      // Сделки больше нет — счёт по ней закрыт. Каким он был, снимок не
      // сообщает: закрытие мы увидели не по исполнению.
      p.realizedPnl = 0;
