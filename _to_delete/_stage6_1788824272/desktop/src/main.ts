...
import { noticeIcon, type NoticeIcon } from './noticeIcon.js';
...
const guard = new Guard(
  ...
  (e) => notifyDetection(e),
  (title, body, icon) => notify(title, body, icon),
);
 ...
 *
 * `icon` не украшение: уведомление читают краем глаза, и цвет со знаком часто
 * оказываются единственным, что человек успевает разобрать. На macOS система
 * собственную иконку игнорирует и рисует иконку приложения — там останется
 * только текст.
 */
function notify(
  title: string,
  ...
  new Notification({ title, body, urgency, icon: noticeIcon(icon) })
    .on('click', () => showWindow())
    .show();
}
...
function notifyDetection(e: GuardEvent): void {
  notify(
    `AntiAvg: ${e.symbol ?? 'позиция'}`,
    `Усреднение в убытке — ${e.text}`,
...
const WEAKENING: WeakeningRule[] = [
  ...
  {
    key: 'fomoMinLossPct',
    label: 'Порог убытка для защиты от FOMO',
    weaker: (b, a) => a.fomoMinLossPct > b.fomoMinLossPct,
  },
  {
    key: 'fomoBurstCount',
    label: 'Сколько убыточных входов за окно включают защиту от FOMO',
    // 0 выключает признак целиком — это ослабление при любом прежнем значении.
    weaker: (b, a) => a.fomoBurstCount > b.fomoBurstCount || (a.fomoBurstCount === 0 && b.fomoBurstCount > 0),
  },
  {
    key: 'fomoBurstWindowSec',
    label: 'Окно набора убыточных входов',
    weaker: (b, a) => a.fomoBurstWindowSec < b.fomoBurstWindowSec,
  },
];
