...
export interface Settings {
  ...
  maxRiskPct: number;
  /** Защита от FOMO: выкл / только сигнал / сигнал с блокировкой торговли. */
  fomoMode: 'off' | 'notify' | 'block';
  /** Окно, в которое должна уместиться серия стоп-аутов, секунды. */
  fomoWindowSec: number;
  /** Сколько стоп-аутов подряд образуют серию. */
  fomoCount: number;
  /** Максимальная длительность одной сделки серии, секунды. */
  fomoTradeSec: number;
  /** Минимальный убыток сделки, % от номинала позиции. */
  fomoMinLossPct: number;
  /** Признак 2: сколько убыточных сделок в окне образуют пачку. 0 — выключен. */
  fomoBurstCount: number;
  /** Окно, в которое должны уложиться открытия убыточных сделок, секунды. */
  fomoBurstWindowSec: number;
  /** Сколько держать блокировку торговли, минуты. */
  fomoBlockMin: number;
  symbols: string;
  ...
  /** Подробный лог: уровень debug и печать каждого сырого события WebSocket. */
  verboseLog: boolean;
}
