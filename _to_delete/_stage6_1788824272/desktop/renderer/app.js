...
function renderState(st) {
  ...
  // Блокировка после FOMO важнее любого другого состояния: пока она держится,
  // человек всё равно попробует войти, и он должен видеть почему не выходит.
  const fomo = st.snapshot && st.snapshot.fomo;
  if (fomo && fomo.blocked) {
    pill.dataset.state = 'alarm';
    const left = Math.max(1, Math.ceil((fomo.blockUntilMs - Date.now()) / 60000));
    $('stateText').textContent = `Торговля заблокирована (FOMO) — ещё ~${left} мин`;
  }
...
const FIELDS = {
  ...
  sFomoMode: 'fomoMode', sFomoCount: 'fomoCount', sFomoWindowSec: 'fomoWindowSec',
  sFomoTradeSec: 'fomoTradeSec', sFomoMinLossPct: 'fomoMinLossPct',
  sFomoBurstCount: 'fomoBurstCount', sFomoBurstWindowSec: 'fomoBurstWindowSec',
  sFomoBlockMin: 'fomoBlockMin',
