...
export interface PositionState {
  ...
  /**
   * Накопленный результат ТЕКУЩЕЙ сделки в валюте котировки: сумма прибыли и
   * убытка по всем закрывающим исполнениям с момента открытия позиции.
   * Обнуляется при открытии и при перевороте.
   *
   * Считается по своей модели, а не по полю `rp` из события биржи: `TRADE_LITE`
   * приходит раньше `ORDER_TRADE_UPDATE`, реализованного PnL не содержит, и
   * именно он часто оказывается тем исполнением, по которому мы принимаем
   * решение. Комиссия сюда НЕ входит — иначе стоп в безубыток всегда выглядел
   * бы убыточным.
   */
  realizedPnl: number;
