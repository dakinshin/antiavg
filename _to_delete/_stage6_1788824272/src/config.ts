...
export const ConfigSchema = z.object({
   ...
   *
   * Второй, независимый признак FOMO. В отличие от серии он не требует ни
   * стопов, ни коротких сделок, ни того, чтобы убытки шли подряд: важна только
   * кучность входов. Ловит того, кто набрал несколько позиций разом и все их
   * потерял — даже если между ними были удачные сделки и держал он их долго.
   */
  fomoBurstWindowMs: numFromEnv(60_000),
  /** Сколько убыточных сделок в этом окне образуют пачку. 0 — признак выключен. */
  fomoBurstCount: numFromEnv(3),
  /**
   * Минимальный убыток сделки, % от номинала позиции. Ниже — считаем, что
   * сделка закрыта в безубыток, и в серию она не идёт.
   *
   * Порог нужен потому, что стоп в безубытке срабатывает на цене входа, но
   * исполняется по рынку: результат выходит чуть отрицательным на
   * проскальзывание. Без порога дисциплинированный выход в ноль записывался бы
   * в FOMO — то есть ровно противоположное поведение.
   */
  fomoMinLossPct: numFromEnv(0.05),
...
export function loadConfigFromEnv(env: NodeJS.ProcessEnv = process.env): Config {
  const raw = {
    ...
    fomoBurstWindowMs: env.ANTIAVG_FOMO_BURST_WINDOW_MS,
    fomoBurstCount: env.ANTIAVG_FOMO_BURST_COUNT,
    fomoMinLossPct: env.ANTIAVG_FOMO_MIN_LOSS_PCT,
