...
/**
 * Сколько после срабатывания стопа не жаловаться на отсутствие защиты.
 *
 * Между исчезновением стопа и приходом закрывающего исполнения позиция
 * выглядит незащищённой, хотя на самом деле она уже закрывается. Кричать в этот
 * промежуток — значит уведомлять человека о том, что защита сработала правильно.
 */
const STOP_TRIGGER_GRACE_MS = 5000;
...
export class RiskGuard {
  ...
  /** Когда по позиции в последний раз сработал стоп. */
  private readonly stopTriggeredAt = new Map<PositionKey, number>();
  /** Отложенные перепроверки после срабатывания стопа. */
  private readonly afterTriggerTimers = new Map<PositionKey, ReturnType<typeof setTimeout>>();
  ...
  onOrderEvent(evt: OrderLifecycleEvent): void {
    ...
    if (!isTerminalStatus(evt.orderStatus)) {
      ...
      if (kind) {
        ...
        // TRIGGERING — переходное состояние: стоп уже сработал, следом придёт
        // TRIGGERED, а за ним исполнение, закрывающее позицию. Проверять по нему
        // риск нельзя: пока проверка ждёт депозит, стоп успевает исчезнуть из
        // открытых ордеров, а позиция ещё числится открытой — и сервис объявляет
        // «риск ничем не ограничен» ровно в тот момент, когда стоп отработал
        // как надо.
        if (evt.orderStatus === 'TRIGGERING') {
          this.stopTriggeredAt.set(key, this.now());
          return;
        }
    ...
    // Стоп сработал. Позиция закрывается прямо сейчас, но исполнение ещё не
    // дошло — на эти несколько сотен миллисекунд она выглядит незащищённой.
    // Отмечаем момент, чтобы не поднимать ложную тревогу, и перепроверяем
    // позже: стоп мог быть частичным, и тогда позиция действительно осталась
    // без защиты — об этом сказать надо.
    if (known && (evt.orderStatus === 'TRIGGERED' || evt.orderStatus === 'FILLED')) {
      this.stopTriggeredAt.set(key, this.now());
      this.scheduleAfterTrigger(order.symbol, order.positionSide);
    }
  ...
  /**
   * Открытый вопрос «наша ли это позиция» — для других модулей.
   *
   * Признак «наша» ведёт этот класс: здесь же он переносится через перезапуск
   * (`seedOwnPositions`) и здесь же сбрасывается при закрытии позиции. Заводить
   * второй такой учёт в защите от FOMO значило бы получить два ответа на один
   * вопрос — и однажды разные.
   */
  isOwn(symbol: string, positionSide: PositionSide): boolean {
    const key = positionKey(symbol, positionSide);
    const pos = this.opts.positions.peek(symbol, positionSide);
    if (!pos || isZero(pos.qty)) {
      // Заодно убираем отметку: правила риска могут быть выключены целиком,
      // и тогда review() до forget() не доходит.
      this.ownPositions.delete(key);
      return false;
    }
    return this.isOwnPosition(pos, key);
  }
  ...
  private forget(key: PositionKey): void {
    ...
    const afterTrigger = this.afterTriggerTimers.get(key);
    if (afterTrigger) {
      clearTimeout(afterTrigger);
      this.afterTriggerTimers.delete(key);
    }
  }
  ...
  /** Стоп по этой позиции сработал только что — позиция закрывается. */
  private stopJustTriggered(key: PositionKey): boolean {
    const at = this.stopTriggeredAt.get(key);
    return at !== undefined && this.now() - at < STOP_TRIGGER_GRACE_MS;
  }

  /**
   * Перепроверка после срабатывания стопа.
   *
   * Обычно к этому моменту позиции уже нет и проверка ничего не делает. Но стоп
   * мог закрывать только часть объёма — тогда позиция жива и действительно
   * осталась без защиты, и молчать об этом нельзя. Без такой перепроверки
   * следующий повод узнать об этом появился бы только на плановой сверке.
   */
  private scheduleAfterTrigger(symbol: string, positionSide: PositionSide): void {
    const key = positionKey(symbol, positionSide);
    const existing = this.afterTriggerTimers.get(key);
    if (existing) clearTimeout(existing);
    const timer = setTimeout(() => {
      this.afterTriggerTimers.delete(key);
      void this.track(this.review(symbol, positionSide, 'после срабатывания стопа'));
    }, STOP_TRIGGER_GRACE_MS + 250);
    if (typeof timer.unref === 'function') timer.unref();
    this.afterTriggerTimers.set(key, timer);
  }
  ...
  private notifyRisk(symbol: string, positionSide: PositionSide, a: RiskAssessment): void {
    ...
    // Стоп только что сработал — позиция закрывается прямо сейчас, просто
    // исполнение ещё не дошло. «Риск ничем не ограничен» в этот момент
    // сообщает человеку, что защита сработала, — ровно наоборот по смыслу.
    if (a.verdict === 'no-stop' && this.stopJustTriggered(key)) {
      this.log.debug('стопа нет, но он только что сработал — позиция закрывается', {
        symbol,
        positionSide,
      });
      return;
  ...
  stop(): void {
    ...
    for (const t of this.afterTriggerTimers.values()) clearTimeout(t);
    this.afterTriggerTimers.clear();
  }
}
