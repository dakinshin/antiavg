...
/** Полное закрытие позиции — то, что для внешнего наблюдателя и есть «сделка». */
...
export interface EngineOptions {
  ...
  hooks?: EngineHooks;
  /**
   * Позиция закрылась полностью. Отдельным колбэком, а не хуком: это не
   * «событие защиты», о котором сообщают человеку, а факт для другого модуля
   * (защиты от FOMO), и смешивать их в одном интерфейсе значило бы навязывать
   * его десктопной обёртке.
   */
  onPositionClosed?: (info: ClosedTradeInfo) => void;
  /**
...
export class Engine {
  ...
  private readonly onPositionClosed: ((info: ClosedTradeInfo) => void) | undefined;
  ...
  onFill(fill: FillEvent): DetectionResult {
    ...
    // Позиция закрылась этим исполнением — то есть сделка завершилась. Время
    // жизни считаем от открытия ПОЗИЦИИ (значение `before`, до применения
    // этого же исполнения: после него оно уже обнулено).
    if (applied.closed && this.onPositionClosed) {
      const closedAtMs = fill.tradeTimeMs || fill.eventTimeMs || this.now();
      const known = before.openTimeKnown && before.openedAtMs !== null;
